...
function writeLog() {
  ...
  if (pendingEvents.length > 0) 
    $.ajax({ type: 'POST', url: "https://services.thegamma.net/services/log/olympics",
...
function saveAndShare()
{
  ...
  $.ajax({
    url: "https://services.thegamma.net/services/snippets/olympics", data: JSON.stringify(data),
    contentType: "application/json", type: "POST", dataType: "JSON"
  }).done(function (res) {
    // Display the confirmation window with links
    var link = "/rio2016/shared/" + res + "/" + title.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"");
    var ut = encodeURIComponent(title);
    var ul = encodeURIComponent("https://rio2016.thegamma.net" + link);
...
function setCurrentSection(top) {
  ...
  if (newSection != currentSection) {
    ...
    history.replaceState({}, "/rio2016/" + selUrl, "/rio2016/" + selUrl);
  }
}
