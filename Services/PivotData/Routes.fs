module TheGamma.Services.PivotData.Routes
...
module Transform =
  ...
  let parseOp nullary unary (op:string) =
    ...
    | Some(_, op) -> op
    | _ ->
      let parsed = unary |> List.tryPick (fun (k:string, f) ->
        if op.StartsWith(k) then Some(f(trimIdent(op.Substring(k.Length + 1))))
        else None)
      if parsed.IsSome then parsed.Value else failwith "Unknown operation"
  ...
  let parseTransform (op, args) =
    ...
    | "groupby", ops ->
        let keys = ops |> List.takeWhile (fun (s:string) -> s.StartsWith("by ")) |> List.map (fun s -> trimIdent (s.Substring(3)))
        let aggs = ops |> List.skipWhile (fun (s:string) -> s.StartsWith("by ")) |> List.map (parseOp groupNullaryOps groupUnaryOps)
        GroupBy(keys, aggs)
...
module Olympics =
  ...
  let allData = lazy (
    let medals = Medals.Load(Path.Combine(dataRoot, "medals-expanded.csv"))
    medals.Rows |> Seq.map (fun r ->
      [| "Games", String r.Games; "Year", Number (float r.Year);  "Sport", String r.Sport
         "Discipline", String r.Discipline; "Athlete", String r.Athlete; "Team", String(countries.Value.[r.Team]);
  ...
  let readFiles () =
    ...
    for f in Directory.GetFiles(cache) do
      ...
      for z in dump.Zaznams do
        ...
        let prijemci = if prijemci.IsEmpty then "Nezadáno" else System.String.Join(", ", prijemci)
        let prijemciIco = if prijemciIco.IsEmpty then "Nezadáno" else System.String.Join(", ", prijemciIco)
...
let initAllData dataRoot =
  Olympics.initData dataRoot
  Smlouvy.initData (Path.Combine(dataRoot, "smlouvy-cache"))

let handler : HttpHandler = routef "/%s" <| fun source -> fun next ctx -> task {
  let source', meta =
  ...
  let qs : string = ctx.Request.QueryString.Value
  let parts = if System.String.IsNullOrEmpty(qs) then [] else qs.TrimStart('?').Split('&') |> Array.map (fun (s:string) -> s.Split('=').[0]) |> List.ofArray
  let preview, query = parts |> List.partition ((=) "preview")
  let isPreview = not (List.isEmpty preview)
  let queryStr = if query.IsEmpty then "" else query |> List.head
  let q = Transform.fromUrl queryStr

  let res = q.Transformations |> List.fold transformData (Seq.ofArray source') |> Array.ofSeq
  let json = applyAction isPreview meta res q.Action
  return! text (json.ToString()) next ctx }
