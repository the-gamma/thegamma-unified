...
let createFacetApp records fields facets : HttpHandler =
    ...
    | _ ->
        let result =
          ...
          |> Seq.map (fun (facetKey, facet) ->
              ...
              { name="by " + facetKey; returns= {kind="nested"; endpoint=localPath </> "pick" </> facetKey}
                schema=schema; trace=[| |]; documentation=doc })
          |> Seq.append [
