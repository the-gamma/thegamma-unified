module TheGamma.Services.Pivot.Routes
...
module Transform =
  ...
  let private parseChunk = function
    ...
    | "group"::args ->
        let fields, aggs = args |> List.partition (fun (s:string) -> s.StartsWith("by-"))
        let fields, aggs =
          match fields, aggs with
          | [], [] -> [], []
          | [], field::aggs -> [field], aggs
          | fields, aggs -> fields |> List.map (fun (s:string) -> s.Substring(3)), aggs
...
let dropParameters (mq:MemberQuery.Root) =
  ...
  | _ -> failwith "dropParameters: expected record"
...
let withThingSchema thingType thingName (m:MemberQuery.Root) =
  ...
  | JsonValue.Record(props) ->
      ...
      let props = Array.append [| ("schema", schema) |] props
      MemberQuery.Root(JsonValue.Record(props))
  | _ -> failwith "withThingSchema: expected record"

let membersOk (s:seq<MemberQuery.Root>) =
  JsonValue.Array([| for a in s -> a.JsonValue |]).ToString()
...
let originalRequest source parts (httpMethod:string) (body:byte[]) = async {
  let another = concatUrl (source::parts)
  let bodyOpt =
    if body.Length = 0 then None
    else Some(HttpRequestBody.BinaryUpload body)
  return!
    Http.AsyncRequestString
      ( another, [], httpMethod = httpMethod, ?body = bodyOpt,
...
let handleDataRequest source next (ctx:HttpContext) = task {
  use reader = new StreamReader(ctx.Request.Body)
  let! bodyStr = reader.ReadToEndAsync()
  let trace = bodyStr.Split('&') |> List.ofSeq
...
// ----------------------------------------------------------------------------
// Transformation request handlers
// ----------------------------------------------------------------------------
...
let handlePagingRequest injectid { Fields = fields } rest pgid ops =
  ...
  ( match ops with
    ...
    | _ -> failwith "handlePagingRequest: Should not happen" ) |> membersOk
...
let handleProxyRequest source local (httpMethod:string) (body:byte[]) = async {
  let! data = originalRequest source local httpMethod body
  let transformed =
...
// ----------------------------------------------------------------------------
// Giraffe handler with source from x-cookie header
// ----------------------------------------------------------------------------

let withSource (f: string -> HttpHandler) : HttpHandler =
  fun next ctx -> task {
    let source =
      ...
      | true, values ->
          ...
          let pairs =
            ...
            |> Array.choose (fun kv ->
              ...
              | [|ck;cv|] -> Some(ck, System.Web.HttpUtility.UrlDecode cv)
              | _ -> None)
          let cookies = dict pairs
          match cookies.TryGetValue("source") with
          | true, sv -> Some sv
          | _ -> None
      | _ -> None
    match source with
    | Some src -> return! f src next ctx
    | None -> return! RequestErrors.BAD_REQUEST "Cookie 'source' was missing." next ctx }

let pivotRequest (f: string -> StoredFields -> Transformation list -> Transformation -> HttpHandler) : HttpHandler =
  routeStartsWith "/pivot/" >=> fun next ctx -> task {
    let fullPath = ctx.Request.Path.Value
    // The endpoint URLs generated by toReturns look like /pivot/{injectid}/{transforms}
    // Find the last occurrence of /pivot/ in the full path to locate the inject ID
    let pivotPrefix = "/pivot/"
    let pivotIdx = fullPath.LastIndexOf(pivotPrefix)
    let afterPivot =
      if pivotIdx >= 0 then fullPath.Substring(pivotIdx + pivotPrefix.Length)
      else fullPath
    let slashIdx = afterPivot.IndexOf('/')
    let injectid, rest =
      if slashIdx < 0 then afterPivot.TrimEnd('/'), ""
      else afterPivot.[..slashIdx-1], afterPivot.[slashIdx+1..].TrimEnd('/')
    ...
    | Some fields ->
        ...
        return! f injectid fields rest current next ctx
    | _ ->
        return! RequestErrors.BAD_REQUEST (sprintf "Injected path does not have valid id: '%s' (fullPath=%s)" injectid fullPath) next ctx }

let handler : HttpHandler =
  withSource (fun source ->
    choose [
      route "/pivot/data" >=> fun next ctx -> handleDataRequest source next ctx
      pivotRequest (fun injectid fields rest current ->
        fun next ctx -> task {
          let result =
            match current with
            | Empty ->
      ...
      routef "/%s" (fun path ->
        fun next ctx -> task {
          let local = path.Split([|'/'|], StringSplitOptions.RemoveEmptyEntries) |> List.ofArray
          use ms = new MemoryStream()
          do! ctx.Request.Body.CopyToAsync(ms)
          let body = ms.ToArray()
          let! result = handleProxyRequest source local ctx.Request.Method body |> Async.StartAsTask
          return! text result next ctx })
      route "/" >=> fun next ctx -> task {
          use ms = new MemoryStream()
          do! ctx.Request.Body.CopyToAsync(ms)
          let body = ms.ToArray()
          let! result = handleProxyRequest source [] ctx.Request.Method body |> Async.StartAsTask
          return! text result next ctx }
    ])
