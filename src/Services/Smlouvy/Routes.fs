module TheGamma.Services.Smlouvy.Routes
...
let readFiles () =
  ...
  for f in Directory.GetFiles(cache) do
    ...
    for z in dump.Zaznams do
      ...
      { DatumPublikace = z.CasZverejneni.DateTime
...
let smlouvy : Lazy<Record[]> = lazy (
  updateCache() |> Async.RunSynchronously
  readFiles() )

let facets : Lazy<list<string * Facet<Record>>> = lazy (
