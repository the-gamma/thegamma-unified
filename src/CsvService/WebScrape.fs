module TheGamma.CsvService.WebScrape
...
let isMonth (monthName:string) =
    ...
    if (matchedMonth.IsEmpty) then None
...
let getDated yyyy (entries:string[] list) =
  entries |> List.collect (fun entry ->
    ...
    if Seq.length listOfWords >= 3 then
      let could1, _ = System.Int32.TryParse(listOfWords.[0])
      if could1 then
        ...
        | Some x ->
          ...
          if could then [entry.[0], entryDate.ToString(), description]
          else []
        | None -> []
      else
        let could2, _ = System.Int32.TryParse(listOfWords.[1])
        if could2 then
...
let getDatedEntries year (url:string) =
  ...
  let csv =
    [ for typ, dt, e in datedEntries -> ExploreDate.Row(typ, dt, System.Text.RegularExpressions.Regex.Replace(e, "\[[0-9]+\]", ""))]
