module TheGamma.CsvService.Pivot
...
module Transform =
  ...
  let parseOp nullary unary (op:string) =
    match nullary |> List.tryFind (fun (k, _) -> k = op) with
    | Some(_, op) -> op
    | _ ->
...
let evalCondition op actual (expected:string) =
  ...
  | InRange, Date dt ->
      ...
      | _ -> failwithf "Value '%s' is not a valid date or time range." expected
  | _, Date dt1 ->
      ...
      | (_, _), InRange -> failwith "evalCondition: Unexpected InRange"
      | (_, _), Like -> failwith "evalCondition: Unexpected Like"
...
let transformData (objs:seq<(string * Value)[]>) = function
  ...
  | WindowBy(fld, size, aggs) ->
      ...
      |> Seq.windowed size
      |> Seq.map (fun win ->
        aggs |> List.collect (applyWinAggregation fld win) |> Array.ofSeq )
  | ExpandBy(fld, aggs) ->
      let funcs = aggs |> Seq.map (getExpandAggregationFunction fld) |> Array.ofSeq
      objs
      |> Seq.sortBy (pickField fld)
      |> Seq.map (fun row -> funcs |> Array.map (fun f -> f row))
  | GroupBy(flds, aggs) ->
      objs
      |> Seq.groupBy (fun j -> List.map (fun f -> pickField f j) flds)
      |> Seq.map (fun (kvals, group) ->
        aggs |> List.collect (applyGroupAggregation (List.zip flds kvals) group) |> Array.ofSeq )
...
let inferType (s:string) =
  if fst (Decimal.TryParse(s)) then Number
...
let readCsvFile (data:string) =
  if System.String.IsNullOrWhiteSpace data then parseError "The specified input was empty."
  ...
  let data =
    ...
    |> Seq.map (fun row ->
      ...
      |> Seq.map (fun ((col, typ), value) ->
        ...
        with _ -> parseError (sprintf "Column '%s' was inferred as %s, but contains non-numeric value '%s'." col (typeName typ) value) )
      |> Array.ofSeq )
    |> Array.ofSeq
...
let handleRequest meta (source:(string * Value)[][]) (query:string list) =
